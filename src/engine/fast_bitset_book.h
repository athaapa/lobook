...
namespace Fast {
    ...
    public:
        ...
        void cancel_order(uint64_t id) {
            ...
            // A: We only want to clear the bitset bit if there are no more active orders at this
            // price level.
            //    We do this after unlinking because then we can simply check price_level.head to
            //    see if there are any orders at this price_level.
        ...
        void replace_order(uint64_t id, uint64_t new_price, uint32_t new_qty) {
            if (id >= id_map.size() || id_map[id] == NULL_IDX)
                return;
            uint32_t order_idx = id_map[id];
            Order& order = orders[order_idx];

            // Case 1: Price change -> lose priority
            uint64_t price = order.price;
            ...
            if (price != new_price) {
                ...
                PriceLevel& new_price_level = levels[new_price];
                if (new_price_level.head == NULL_IDX) {
                    new_price_level.tail = order_idx;
                    new_price_level.head = order_idx;
                    order.prev = NULL_IDX;
                } else {
                    order.prev = new_price_level.tail;
                    Order& tail = orders[new_price_level.tail];
                    tail.next = order_idx;
                    new_price_level.tail = order_idx;
                }
                auto& new_index = order.is_buy ? bid_index : ask_index;
                ...
                Order& tail = orders[price_level.tail];
                tail.next = order_idx;
                price_level.tail = order_idx;
                order.next = NULL_IDX;
            }
        ...
        void add_order(uint64_t id, uint64_t price, uint32_t qty, bool is_buy) {
            ...
            if (level.head == NULL_IDX) {
                ...
                //    Therefore, we don't need to set the bit for every order, only on the first
                //    order.
                index.set(price);
            } else {
    ...
    private:
        ...
        //    different order lists. It isn't useful for an incoming buy order to know that there
        //    are other buys at the same price level.
