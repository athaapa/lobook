...
namespace Naive {
    ...
    public:
        ...
        void add_order(uint64_t id, uint64_t price, uint32_t qty, bool is_buy) {
            ...
            // A: The order only needs to rest (i.e. enter the book) if it can't immediately find a
            // pair
            match(ord);
        ...
        void cancel_order(uint64_t id) {
            ...
            if (it_lookup != order_lookup.end()) {
                ...
                // A: unordered_map.erase(iterator*) is O(1) time complexity. Conversely, erase(key)
                // is average
                //    O(1), but has a worst case of O(n), particularly if there are a lot of hash
                //    collisions.
        ...
        void debug_print() {
            ...
            for (auto const& [price, list] : bids) {
                std::cout << "Bid Price " << price << " has " << list.size() << " orders.\n";
            }
        }
    ...
    private:
        ...
        //    sorted by their lowest sell price. This is helpful for matching (we want to get the
        //    best deal)
        ...
        void process_list(std::list<Order>& order_list, Order& incoming) {
            while (incoming.qty > 0 && !order_list.empty()) {
                ...
                // Q: Why trade std::min(incoming.qty, resting_order.qty) instead of just
                // incoming.qty? A: To allow for partial fills. That way, a resting order can
                // partially fill an incoming one.
        ...
        void match(Order& incoming) {
            if (incoming.is_buy) {
                while (incoming.qty > 0 && !asks.empty()) {
                    ...
                    // Q: Why must we erase the price level from the map when its list becomes
                    // empty? A: So we don't try and match incoming orders at a price_level with no
                    // resting orders.
                    if (order_list.empty())
                        asks.erase(it);
                }
