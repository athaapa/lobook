...
namespace {
    ...
    const BookOps fast_ops { "FastBook (linear scan)", FastBookRunner::init,
        FastBookRunner::submit_order, FastBookRunner::cancel_order, FastBookRunner::get_quantity };

    const BookOps bitset_ops { "BitsetBook (hierarchical bitset)", BitsetBookRunner::init,
        BitsetBookRunner::submit_order, BitsetBookRunner::cancel_order,
    ...
    void scenario_wide(const BookOps& ops) {
        ...
        lobook::bench::print_kv("setup",
            std::to_string(kWideOrders) + " asks at top, then " + std::to_string(kWideOrders)
                + " buys crossing 1-by-1");
