...
inline long perf_event_open(struct perf_event_attr* attr, pid_t pid, int cpu,
...
public:
    ...
    void print_summary(uint64_t num_ops) {
        ...
        os << "      instructions  " << std::setw(8) << std::setprecision(2) << (insn / n)
           << " /op   cycles  " << std::setw(7) << (cyc / n) << " /op   IPC  " << std::setprecision(2)
           << ipc << "\n";
        os << "      branches      " << std::setw(8) << (br / n) << " /op   misses  "
           << std::setprecision(2) << br_miss_pct << " %\n";
        os << "      L1D misses    " << std::setw(8) << std::setprecision(2) << (l1m / n) << " /op\n";
